...
class TestIdentifyUnpairedRightSingleQuoteModuleLevel:
    ...
    def test_word_period_single_quote_inside_double_quotes(self, locale_id):
        """Inside double quotes: word + period + single quote → period moves outside.

        Exception: For rue locale, the swap pattern doesn't match (rightSingleQuote=›
        but converted char=apostrophe), so period stays before apostrophe.
        """
        q = get_quotes(locale_id)
        text = f"{q['ldq']}word.'{q['rdq']}"
        result = fix_single_quotes_primes_and_apostrophes(text, locale_id)
        if locale_id == "rue":
            # rue: pattern doesn't match, period stays before apostrophe
            expected = f"{q['ldq']}word.{APOSTROPHE}{q['rdq']}"
        else:
            # Other locales: terminal punctuation moves outside single quote
            expected = f"{q['ldq']}word{APOSTROPHE}.{q['rdq']}"
        assert result == expected

    @pytest.mark.parametrize("locale_id", ALL_LOCALES)
    def test_word_exclamation_single_quote_inside_double_quotes(self, locale_id):
        """Inside double quotes: word + exclamation + single quote → exclamation moves outside.

        Exception: For rue locale, the swap pattern doesn't match (rightSingleQuote=›
        but converted char=apostrophe), so exclamation stays before apostrophe.
        """
        q = get_quotes(locale_id)
        text = f"{q['ldq']}word!'{q['rdq']}"
        result = fix_single_quotes_primes_and_apostrophes(text, locale_id)
        if locale_id == "rue":
            # rue: pattern doesn't match, exclamation stays before apostrophe
            expected = f"{q['ldq']}word!{APOSTROPHE}{q['rdq']}"
        else:
            # Other locales: terminal punctuation moves outside single quote
            expected = f"{q['ldq']}word{APOSTROPHE}!{q['rdq']}"
